...
export async function temporaryDirectory(prefix = "wtx-test-"): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), prefix));
}

export async function initialiseRepo(directory: string): Promise<void> {
  await execa("git", ["init", "-b", "main"], { cwd: directory });
  await execa("git", ["config", "user.email", "wtx@example.test"], { cwd: directory });
  await execa("git", ["config", "user.name", "WTX Test"], { cwd: directory });
